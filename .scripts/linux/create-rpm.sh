...
if [ ! -f "system-bridge-linux" ]; then
  echo "system-bridge-linux not found, please build the application first (e.g. 'mise run build:all')" >&2
  exit 1
fi
...
install -Dm755 system-bridge-tui "$STAGING_DIR/usr/bin/system-bridge-tui"
