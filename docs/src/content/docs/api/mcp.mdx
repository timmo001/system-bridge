...
### Quick setup with deep links

Some clients support one-click MCP registration. Replace `YOUR_TOKEN` with your API token.

**Claude Desktop** — paste into your browser:

```text
claude://addServer/system-bridge?transport=websocket&url=ws://localhost:9170/api/mcp?token=YOUR_TOKEN
```

**Cursor** — requires `websocat` (see above). Paste into your browser:

```text
cursor://addServer/system-bridge?command=websocat&args=ws://localhost:9170/api/mcp?token=YOUR_TOKEN
```

For manual config file paths and other clients, see `backend/mcp/README.md` in the repository.
